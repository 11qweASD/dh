...
$(document).ready(function() {
    ...
    $('#search-text').focus(function() {
        $(this).css('box-shadow', '0 0 15px rgba(78,110,242,0.5)');
    }).blur(function() {
    ...
    $(window).scroll(function() {
        ...
        var opacity = Math.min(0.8, 0.3 + scrollTop/500);
...
$(document).ready(function() {
    ...
    function resizeBackground() {
        var $banner = $('.banner-video');
        var $img = $banner.find('img, video');
    ...
    $(window).scroll(function() {
        var scrollTop = $(window).scrollTop();
        var opacity = Math.min(0.5, 0.2 + scrollTop/500);
        $('.navbar').css('background', 'rgba(0,0,0,' + opacity + ')');
    });
    ...
    // 点击"取消"按钮时的处理（假设您有一个取消按钮）
    // 如果您有具体的取消按钮，请提供其ID或类名
    // 这里是一个通用示例：
    /*
    $('.cancel-btn').click(function(e) {
        e.preventDefault();
        // 取消操作，保持背景满屏透明
        $('.banner-video').css('z-index', '-1');
        $('.bottom-cover').hide();
    });
    */
});
